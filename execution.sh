...
if [ $? -ne 0 ]
then 
echo -e "$Y Installing MySQL wait----$N " | tee -a $Log_file
dnf install mysql -y
VALIDATE $? "mysql"
else
...
if [ $? -ne 0 ]
then 
echo -e "$Y Installing nginx wait----$N" | tee -a $Log_file
dnf install nginx -y
VALIDATE $? "nginx"
else
echo -e "$Y Already nginx Installed--$N" | tee -a $Log_file
fi
...
if [ $? -ne 0 ]
then 
echo -e "$Y Installing git wait----$N" | tee -a $Log_file

dnf install git -y
VALIDATE $? "git"
else
echo -e "$Y Already git Installed--$N" | tee -a $Log_file
fi
